        ...
        <AnimatePresence>
            {isScrolling ? (
                ...
                >
                    <div className="items-center hidden gap-5 md:flex">
                           {dataHeader.map(({ id, name, link}) =>(
                                <Link key={id} href={link} className="hover:text-slate-700 hover:border-b-[1px] hover:-text-slate-700">{name}</Link>            
                           ))} 

                           <Link href="/login" className="px-3 py-2 text-white rounded-lg bg-slate-700 hover:bg-black">login</Link>
                    </div>
                ...
                <div className={`${openMobileMenu ? 'absolute z-[1] left-0 top-20 bg-white r-0 w-full px-4 py-4' : 'hidden'} gap-5 md:flex`}>
                    {dataHeader.map(({ id, name, link }) => (
                        <Link key={id} href={link} className="block hover:text-slate-700 hover:boder-b-[1px]">{name}
